...
def project_id():
    ...
    response = client.post("/v1/projects", json={"name": f"investigate-{os.urandom(4).hex()}", "demo_mode": True})
    project_id = response.json()["project_id"]
    client.post(f"/v1/projects/{project_id}/ingest", json={"path": FIXTURE_PATH})
    return project_id
